    ...
    if (VIDEOS.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center h-full p-8">
                ...
                <h2 className="text-2xl font-bold text-slate-300 mb-4">Brak wykładów</h2>
                <p className="text-slate-500 text-center max-w-md">
                    Dodaj pliki .mp4 do folderu <code className="bg-slate-700 px-2 py-1 rounded">public/videos/</code>
                    <br />i zaktualizuj listę VIDEOS w <code className="bg-slate-700 px-2 py-1 rounded">VideoPlayer.tsx</code>
                </p>
            </div>
        );
    }
                ...
                <div className="w-64 flex-shrink-0 space-y-2 overflow-y-auto">
                    {VIDEOS.map((video) => (
                        ...
                        >
                            ...
                            {video.description && (
                                <p className="text-xs text-slate-500 ml-6">{video.description}</p>
                            )}
                        </button>
                    ))}
                </div>
